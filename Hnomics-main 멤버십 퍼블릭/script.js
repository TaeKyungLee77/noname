...
async function connect() {
    ...
    caver.klay.getBalance(account)
        .then(function (balance) {
            document.querySelector(".wallet_addr").innerHTML = account.substr(0,8) + "...";
            document.querySelector(".balance_title").innerHTML = "Balance(" + account.substr(0,8) + "...)";
            document.querySelector(".balance_count").innerHTML = Math.floor(caver.utils.fromPeb(balance, "KLAY")) + " KLAY";
        });
...
async function check_status() {
    ...
    await projectContract.methods.mintingInformation().call()
        .then(function (res) {
            ...
            document.querySelector(".mintingstart_desc").innerHTML ='<span class="highlight">' + "#" + mintStartBlock + '</span>';
            document.querySelector(".pertransaction_desc").innerHTML = '<span class="small">Max </span>' + mintLimitPerBlock + '</div>';
            document.querySelector(".perwallet_desc").innerHTML = '<span class="small">Max </span>' + mintLimitPerSale + '</div>';
            document.querySelector(".remaining_desc").innerHTML = '<span class="highlight">' + (maxAmount - mintIndexForSale + 1) + '</span><span class="small"> / ' + 1000 + '</span></div>';
            ...
            document.querySelector(".price_count").innerHTML = Math.floor(caver.utils.fromPeb(mintPrice, "KLAY")) * amountCount.value + " KLAY";
        })
        .catch(function (err) {
            console.log(err);
        });
...
async function publicMint() {
    ...
    try {
        const gasAmount = await projectContract.methods.publicMint(amount).estimateGas({
            from: account,
            gas: 6000000,
            value: total_value
        })
...
function mintingSucceed(){
    let heartbeat = document.querySelector(".heartbeat");
    heartbeat.style.backgroundImage = "url('sprites/nft.gif')";
    heartbeat.style.borderRadius = "16px";
    heartbeat.style.boxShadow = "0px 0px 64px #f050a0"
    document.querySelector(".opensea").classList.add("minting_success");
...
setTimeout(() => {
    document.querySelector('iframe').style.display = "none";}, 5000);
